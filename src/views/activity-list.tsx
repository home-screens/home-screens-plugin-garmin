...
export function ActivityListView({ data, units, timezone, activityCount, height, sportFilter }: ViewProps) {
  ...
  const row = (a: GarminActivity) => {
    ...
    return (
      <div key={a.id} style={{
        display: 'flex', alignItems: 'center', gap: u(12), padding: `${u(10)}px ${u(0)}`,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {hidden > 0 && (
        <div style={{ opacity: 0.5, fontSize: u(12), paddingTop: u(8) }}>+{hidden} more</div>
      )}
    </div>
  );
}
