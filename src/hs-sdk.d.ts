...
interface PluginFetchOptions {
  ...
  cacheTtlMs?: number;
  /** When true, the host skips injecting a managed auth token for this request. */
  skipAuth?: boolean;
}
...
declare global {
  interface Window {
    ...
    __HS_SDK__?: {
      ...
      // ── Host-Managed Auth ──
      /** Whether the host holds a live token for this plugin's auth adapter.
       *  Absent on hosts older than the auth-adapter release — guard every call. */
